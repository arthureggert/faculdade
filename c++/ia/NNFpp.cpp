...
NNFpp::NNFpp(int layrs, int neurs[], int range, int A_Function,
		float FunctionParameter, float Epsilon) {
	InitType = 1;
	layers = layrs;
	neurons = (int*) malloc(layrs * sizeof(int));
	bias = (float**) malloc(layrs * sizeof(float*));
...
NNFpp::NNFpp(FILE *NNFfile) {
	...
	if (strcmp(str, "MLP") == 0) {
		...
		for (int i = 0; i < layers; i++) {
			...
			if (i) {
				for (int j = 0; j < neurons[i]; j++) {
					weight[i][j] = (float*) malloc(
							neurons[i - 1] * sizeof(float));
				}
			}
		}
...
void NNFpp::BackPropagateOnLine(float desired[]) {
	...
	for (int i = layers - 1; i > 0; i--) {
		for (int j = 0; j < neurons[i]; j++) {
			for (int k = 0; k < neurons[i - 1]; k++) {
				this->weight[i][j][k] += epsilon * delta[i][j]
						* value[i - 1][k];
			}
...
float NNFpp::LearningBP(int i, float err, int l) {
	...
	case NNF_SIGMA_HYPERBOLIC:
		...
		break;
	case NNF_SIMIL_SIGMOID:
		delta[l][i] = err * (value[l][i] - 0.5)
				* (abs(value[l][i] - 0.5) + 0.5);
		break;
	case NNF_SINC:
		if (value[l][i] > 0) {
			delta[l][i] = err * value[l][i] * (1 - value[l][i]);
...
void NNFpp::TrainingSetFromFile(FILE *TrainingSet, int seed) {
	...
	for (int i = 0; i < TD->ExamplesNum; i++) {
		...
		Errors->ErrorQuadratic(out, (float*) TD->Data[TD->Sequence[i]].Des,
				TD->NeuDes);
		Errors->Accuracy(out, (float*) TD->Data[TD->Sequence[i]].Des,
				TD->NeuDes);
	}
}
...
void NNFpp::TestOnFile(FILE *test, FILE *output) {
	...
	for (int i = 0; i < Examples; i++) {
		...
		Errors->ErrorQuadratic(out, des, out_N);
		Errors->Accuracy(out, des, out_N);
		fprintf(output, "%d\t%f\t%d\n", i, Errors->SingleExampleError,
				Errors->SingleExampleAccuracy);
	}
	fprintf(output,
			"\nExamples: %d\nCumulative Quadratic Error: %f \nAccuracy on Test Set: %f \%\n",
			Examples, Errors->CumulativeError,
			((Errors->CumulativeAccuracy / (float) Examples) * 100));
	fseek(test, 0, 0);
}
