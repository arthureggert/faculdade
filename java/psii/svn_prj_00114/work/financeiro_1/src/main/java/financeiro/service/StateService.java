...
public class StateService {
	...
	@Inject
	private StateRepository empRepo;
	...
	public void registerState(State state, String username, String password,
			String... roles) {
		User user = new User();
		user.setLoginName(username);
		user.setAttribute(new Attribute<Long>(MEMBER_ID_ATTRIBUTE, state
				.getId()));
	...
	@Transactional(qualifier = PicketLink.class)
	public void changePassword(State emp, String password) {
		User user = getUserForState(emp);
		idm.updateCredential(user, new Password(password));
	}

	@Transactional(qualifier = PicketLink.class)
	public void setRoles(State state, String... roles) {
		setRoles(getUserForState(state), roles);
	...
	public List<String> getRoles(State employee) {
		User user = getUserForState(employee);
