...
public class InfIniciais extends JFrame {
	...
	public InfIniciais(int numProd, int numArmaProd, List<Integer> numConsProds, List<Integer> numArmaConsProds, int numCons) throws InterruptedException {
		super("Informa��es Iniciais");
		this.listaProdCons = new ArrayList[numConsProds.size()];
		this.listaProd = new ArrayList<Processo>();
		this.listaCons = new ArrayList<Processo>();
	...
	private void imprimeRelatorio() {
		try {
			for (Processo p : this.listaProd) {
				p.join();
			}
			for (int i = 0; i < this.listaProdCons.length; i++) {
				for (Processo p : this.listaProdCons[i]) {
					p.join();
				}
			}
			for (Processo p : this.listaCons) {
				p.join();
			}
	...
	private void initComponents(int numProd, int numArmaProd, List<Integer> numConsProds, List<Integer> numArmaConsProds, int numCons) throws InterruptedException {
		...
		JLabel lblinhaProd1 = new JLabel("Linha de s�rie 1");
		...
		for (int y = 0; y < numConsProds.size(); y++) {
			JLabel lblinhaProd2 = new JLabel("Linha de s�rie " + (y + 2));
			...
			this.listaProdCons[y] = new ArrayList<Processo>();
			for (int i = 0; i < numConsProds.get(y); i++) {
				...
				this.listaProdCons[y].add(prodCons);
		...
		JLabel lblinhaProd3 = new JLabel("Linha de s�rie " + num);
