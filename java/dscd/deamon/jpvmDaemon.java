...
class jpvmTaskList {
	...
	public void deleteTask(jpvmTaskId tid) {
		...
		jpvmTaskListRecord tmp = this.tasks;

		// Check head
		if(tmp.tid.equals(tid)) {
			if (this.iter == tmp) this.iter=tmp.next;
		...
		while(tmp.next != null) {
			if(tmp.next.tid.equals(tid)) {
				if(this.iter==tmp.next) this.iter=tmp.next.next;
	...
	public jpvmTaskListRecord firstIter() {
		...
		jpvmTaskListRecord ret = this.tasks;
	...
	public jpvmTaskListRecord nextIter() {
		...
		jpvmTaskListRecord ret = this.iter;
...
class jpvmSpawnWorkOrderList {
	...
	public jpvmSpawnWorkOrder newOrder(int num) {
		...
		ret = new jpvmSpawnWorkOrder(this.nextOrder, num);
		this.nextOrder ++;
		ret.next = this.list;
		this.list = ret;
		return ret;
...
class jpvmExecTaskThread extends Thread {
	...
	private boolean doExec() {
		try {
			...
			buf.pack(this.order.order);
			buf.pack(-1);
			try {
				this.jpvm.pvm_send(buf,this.client,
						jpvmDaemonMessageTag.jpvmdCreatedTask);
			}
			catch (jpvmException jpe) {
				perror("CreateTask, \""+jpe+" sending "
						+ "to client " + this.client.toString());
			}
			return false;
		}
		return true;
	...
	public void setDebug_on(boolean debug_on) {
