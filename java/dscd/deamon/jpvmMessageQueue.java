...
class jpvmMessageQueue {
	...
	private synchronized void addElement(jpvmMessageQueueElement nw) {
		if(this.list_head == null) {
			this.list_head = this.list_tail = nw;
			return;
		}
	...
	private synchronized void deleteElement(jpvmMessageQueueElement d) {
		...
		if(this.list_head == d) {
			...
			this.list_head = d.next;
			if(this.list_tail == d) this.list_tail=null;
			return;
		}
		jpvmMessageQueueElement tmp = this.list_head;
		while(tmp.next != d) {
			tmp = tmp.next;
			if(tmp == null) {
		...
		tmp.next = d.next;
		if(this.list_tail==d) this.list_tail=tmp;
	}
